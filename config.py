...
class Config:
    ...
    DEVICE_MANUAL_RECONCILER_POLL_MS = _env_int("DEVICE_MANUAL_RECONCILER_POLL_MS", 1000)
    ...
    RECIPE_PROGRAM_DEVICE_CONNECT_TIMEOUT_MS = _env_int("RECIPE_PROGRAM_DEVICE_CONNECT_TIMEOUT_MS", 3000)
    RECIPE_PROGRAM_DEVICE_RESPONSE_TIMEOUT_MS = _env_int("RECIPE_PROGRAM_DEVICE_RESPONSE_TIMEOUT_MS", 1200)
    RECIPE_PROGRAM_DEVICE_WRITE_TIMEOUT_MS = _env_int("RECIPE_PROGRAM_DEVICE_WRITE_TIMEOUT_MS", 1200)
    RECIPE_PROGRAM_DEVICE_MAX_RETRIES = _env_int("RECIPE_PROGRAM_DEVICE_MAX_RETRIES", 1)
    RECIPE_HUBER_SETPOINT_MIN_INTERVAL_SECONDS = _env_float("RECIPE_HUBER_SETPOINT_MIN_INTERVAL_SECONDS", 10.0)
    RECIPE_HUBER_SETPOINT_MIN_DELTA_C = _env_float("RECIPE_HUBER_SETPOINT_MIN_DELTA_C", 0.2)
    RECIPE_IKA_RPM_MIN_INTERVAL_SECONDS = _env_float("RECIPE_IKA_RPM_MIN_INTERVAL_SECONDS", 5.0)
    RECIPE_IKA_RPM_MIN_DELTA = _env_float("RECIPE_IKA_RPM_MIN_DELTA", 10.0)
    RUNTIME_COMMAND_WORKER_COUNT = _env_int("RUNTIME_COMMAND_WORKER_COUNT", 3)
    APP_PASSWORD_HASH = os.getenv("APP_PASSWORD_HASH", "")
    SESSION_LIFETIME_HOURS = _env_int("SESSION_LIFETIME_HOURS", 8)
    SESSION_COOKIE_SECURE = _env_bool("SESSION_COOKIE_SECURE", False)
    MEASUREMENT_POLLER_INTERVAL_SECONDS = _env_int("MEASUREMENT_POLLER_INTERVAL_SECONDS", 10)
    MEASUREMENT_POLLER_BACKGROUND_HUBER_ENABLED = _env_bool("MEASUREMENT_POLLER_BACKGROUND_HUBER_ENABLED", False)
    HUBER_POLLER_INTERVAL_MS = _env_int("HUBER_POLLER_INTERVAL_MS", 3000)
    ICS435_POLLER_INTERVAL_MS = _env_int("ICS435_POLLER_INTERVAL_MS", 500)
    ICS435_RESPONSE_TIMEOUT_MS = _env_int("ICS435_RESPONSE_TIMEOUT_MS", 1200)
    ICS435_CONNECT_TIMEOUT_MS = _env_int("ICS435_CONNECT_TIMEOUT_MS", 3000)
    ICS435_WRITE_TIMEOUT_MS = _env_int("ICS435_WRITE_TIMEOUT_MS", 1200)
    ICS435_MAX_RETRIES = _env_int("ICS435_MAX_RETRIES", 1)
    ...
    # poll (cheap, single-row UPDATE). Persist history at a visible live cadence
    # while still keeping the write rate bounded.
    ICS435_MEASUREMENT_PERSIST_INTERVAL_SECONDS = _env_int("ICS435_MEASUREMENT_PERSIST_INTERVAL_SECONDS", 1)
    ...
    MEASUREMENT_RETENTION_DAYS = _env_int("MEASUREMENT_RETENTION_DAYS", 0)
