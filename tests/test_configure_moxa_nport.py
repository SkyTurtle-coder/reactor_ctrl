...
class ConfigureMoxaNPortTests(unittest.TestCase):
    ...
    def test_ministat_cc_preset_applies_pp_rs232_settings_and_device_protocol(self):
        argv = [
            "--host",
            "192.168.1.50",
            "--device-preset",
        ...
        self.assertEqual(args.stop_bits, 1)
        self.assertEqual(args.flow_control, "none")
        self.assertEqual(args.read_timeout_ms, 1200)
        self.assertEqual(args.write_timeout_ms, 1200)
        self.assertEqual(args.device_protocol, "huber_ministat_cc")
        self.assertEqual(args.device_type, "thermostat")

    def test_ministat_cc_bind_device_targets_moxa_port_four(self):
        calls = []
        ...
        def fake_request_json(*, base_url, path, method="GET", token=None, payload=None):
            ...
            if path == "/api/device-connections" and method == "POST":
                return 201, {"connection_id": 44, **payload}
            if path == "/api/devices" and method == "GET":
                return 200, {"items": []}
            if path == "/api/devices" and method == "POST":
                return 201, {"device_id": 55, "current_binding": None, **payload}
            if path == "/api/devices/55/binding" and method == "PUT":
                return 200, {
                    "device_id": 55,
                    "asset_serial": "MOXA-01-P4-HUBER-MINISTAT-CC",
                    "display_name": "Huber Ministat cc",
                    "protocol": "huber_ministat_cc",
                    "current_binding": {
                        "connection_id": payload["connection_id"],
                        "connection": {
                            "connection_label": "Port 4",
                        },
                    },
                }
        ...
        self.assertEqual(connection_payload["port_number"], 4)
        self.assertEqual(connection_payload["tcp_port"], 4004)
        ...
        self.assertEqual(device_payload["display_name"], "Huber Ministat cc")
        self.assertEqual(device_payload["device_type"], "thermostat")
        self.assertEqual(device_payload["protocol"], "huber_ministat_cc")

        binding_payload = next(call["payload"] for call in calls if call["path"] == "/api/devices/55/binding")
        self.assertEqual(binding_payload["connection_id"], 44)
