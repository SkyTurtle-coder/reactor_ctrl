-- ============================================================
-- reactor_ctrl — vollständiger Datenbank-Reset
-- WARNUNG: Alle Daten werden unwiderruflich gelöscht!
-- ============================================================
-- Verwendung:
--   mysql -u reactor_user -p < sql/db_reset.sql
-- oder als root:
--   mysql -u root -p < sql/db_reset.sql
-- ============================================================
...
DROP DATABASE IF EXISTS reactor_ctrl;

CREATE DATABASE reactor_ctrl
...
-- ============================================================
-- Tabellen in Dependency-Reihenfolge anlegen (Parents zuerst)
-- ============================================================
...
-- control_command ist Parent von control_command_event (FK-Reihenfolge kritisch)
...
-- control_command_event: Child von control_command — command_id muss existieren
...
CREATE TABLE device_manual_state (
  ...
  CONSTRAINT chk_manual_state_queue_status
    CHECK (queue_status IN ('idle', 'queued', 'running', 'error')),
  CONSTRAINT chk_manual_state_active_control_sensor
    CHECK (active_control_sensor IS NULL OR active_control_sensor IN ('internal', 'external')),
...
CREATE TABLE soft_sensor_model (
...
CREATE TABLE soft_sensor_estimate (
...
-- ============================================================
-- Performance-Indizes (werden auch von _initialize_database_schema angelegt)
-- ============================================================
...
CREATE INDEX ix_control_command_requested_at
  ON control_command (requested_at);

CREATE INDEX ix_control_command_event_created_at
  ON control_command_event (created_at);

CREATE INDEX ix_recipe_program_run_finished_started
  ON recipe_program_run (finished_at, started_at);

-- ============================================================
-- View: Letzter Messwert pro Kanal (für Dashboard-Anzeigen)
-- ============================================================
...
-- Nächste Schritte:
--   1. App neu starten (AUTO_CREATE_SCHEMA=true)
--   2. Moxa NPort über configure_moxa_nport.py neu konfigurieren
--   3. Geräte neu anlegen: Huber Unistat 430, IKA 60
--   4. Flowsheet-Build im Reactor Builder neu erstellen
-- ============================================================
