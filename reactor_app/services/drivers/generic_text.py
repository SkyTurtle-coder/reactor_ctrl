...
class GenericTextDriver(DeviceDriver):
    ...
    def execute(self, *, transport: ITransport, request: DeviceCommandRequest) -> DeviceCommandResult:
        ...
        line_ending_name = _coerce_line_ending(payload.get("line_ending"), field_name="line_ending", default="crlf")
        expect_response = _coerce_bool(payload.get("expect_response"), field_name="expect_response", default=True)
        response_terminator_name = _coerce_line_ending(
            payload.get("response_terminator"),
            field_name="response_terminator",
            default=line_ending_name,
        )
        ...
        try:
            request_bytes = str(text).encode(encoding) + _LINE_ENDINGS[line_ending_name]
