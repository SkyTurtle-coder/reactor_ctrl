...
def load_batched_device_plot_series_window(
    *,
    ...
    # Fallback: for specs with no data in the selected time window, re-query the
    # full 30-day history so that all available measurements are returned
    # (bucketed to max_points).  This way the chart never shows empty for a
    # reachable device even when the selected range has no data yet.
    empty_specs = [
    ...
    if empty_specs:
        ...
        fallback_bucket = _bucket_seconds(
            since_minutes=_FALLBACK_DAYS * 24 * 60,
            max_points=max_points,
        )
        try:
            if dialect_name == "mysql":
                history = _load_batched_plot_series_mysql(
                    specs=empty_specs,
                    window_start=fallback_start,
                    window_end=normalized_window_end,
                    bucket_seconds=fallback_bucket,
                )
                if not _batched_series_has_points(history):
                    history = _load_batched_plot_series_python(
                        specs=empty_specs,
                        window_start=fallback_start,
                        window_end=normalized_window_end,
                        bucket_seconds=fallback_bucket,
